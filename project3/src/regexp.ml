...
let regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
  let rec reg_nfa_aux r nfa s x = match r with 
	  |Empty_String -> {
      q0 = nfa.q0; 
      fs = nfa.fs; 
      qs = nfa.qs; 
      sigma = nfa.sigma; 
      delta = nfa.delta @ [s, None, x];
      } 
	  |Union (x1,x2) -> reg_nfa_aux x2 (reg_nfa_aux x1 nfa s x) s x 
    |Char x3 -> {
      q0 = nfa.q0; fs = nfa.fs; qs = nfa.qs; 
      sigma = if List.mem x3 nfa.sigma then nfa.sigma else nfa.sigma @ [x3]; 
      delta = nfa.delta @ [s, Some x3, x] 
      } 
    |Star x4 -> reg_nfa_aux x4 {
      q0 = nfa.q0; 
      fs = nfa.fs; 
      qs= nfa.qs @ [(List.length nfa.qs); (List.length nfa.qs + 1)]; 
      sigma = nfa.sigma; 
      delta = nfa.delta @ [s,None,List.length nfa.qs;
        (s, None, x); 
        (List.length nfa.qs + 1, None, List.length nfa.qs);
        (List.length nfa.qs + 1, None,x)];} (List.length nfa.qs)(List.length nfa.qs + 1) 
    |Concat (x5,x6) -> reg_nfa_aux x6 (reg_nfa_aux x5 {
      sigma = nfa.sigma; 
      delta = nfa.delta; 
      q0 = nfa.q0; 
      fs = nfa.fs; 
      qs = nfa.qs @ [List.length nfa.qs]; } s (List.length nfa.qs)) (List.length nfa.qs) x in 
	    reg_nfa_aux regexp {
        q0 = 0; 
        fs = [1]; 
        sigma = []; 
